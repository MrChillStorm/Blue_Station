...
so, and a new address looks like a new device. A device that advertises
the same name as a known one counts as known, which catches the named
ones. The rest come back as new after a while, so a baseline works best
for a short watch, or for devices that keep their address."""
...
class Baseline:
    ...
    def is_new(self, device) -> bool:
        return device.address not in self.addresses and _name(device) not in self.names
    ...
    def arrivals(self, now: float, devices, near=None) -> list:
        ...
        due = []
        for d in devices:
            if d.address in self._announced or not self.is_new(d):
                continue
    ...
    def count_new(self, devices, now: float) -> int:
        return sum(1 for d in devices if self.is_new(d) and now - d.last_seen <= GONE_AFTER)
